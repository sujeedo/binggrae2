...
// 색종이 이미지를 아래로 반복 슬라이드하여 흩날리는 효과를 줍니다.
let wh = window.outerHeight
let con_img = document.querySelectorAll('#contetti>li');
con_img[0].style.height = wh+'px'
con_img[1].style.height = wh+'px'
con_img[2].style.height = wh+'px'
$(function(){
    $(con_img).not(':first').css('display','none');
    contettiSilding();
    
    function contettiSilding(){
        $(con_img).eq(0).animate({'top': wh+'px'},3000,'linear');
        $(con_img).eq(1).css('display','block').css('top','-'+wh+'px').animate({'top':'0px'},3000,'linear',
        function(){
        $(con_img).eq(1).animate({'top': wh+'px'},3000,'linear');
        $(con_img).eq(2).css('display','block').css('top','-'+wh+'px').animate({"top":"0px"},3000,'linear',
        function(){
        $(con_img).eq(2).animate({'top': wh+'px'},3000,'linear');
        $(con_img).eq(0).css('display','block').css('top','-'+wh+'px').animate({"top":"0px"},3000,'linear',contettiSilding);
        });
        });
    }
});

// 페이지 로딩이 완료시 커텐이 걷히는 효과를 줍니다.
$(document).ready(function() {
  $('.loading').show();
});
$(window).on('load', function() {
  $('.loading>p').eq(1).css("margin-left","1920px").css('transition','1s'); 
  $('.loading_item').hide();
});
setTimeout(function(){
	jQuery('.loading').css('z-index','-100');
},1000);
...
/*업다운 화살표 버튼에 동작을 줍니다.*/
// 1. 하단의 업다운버튼을 클릭하면 맨위로 맨아래로 이동합니다.
...
// 2. 업다운버튼이 홈의 중앙을 벗어나면 부드럽게 나타납니다.
...
// 홈 하단의 이미지 버튼을 클릭하면 해당 섹션으로 스크롤링하여 이동합니다. 
...
// 상품 옆 버튼 클릭시 메모 팝업과 아이콘이 +에서 -모양으로 변환됩니다.
